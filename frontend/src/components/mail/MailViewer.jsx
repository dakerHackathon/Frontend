...
const MailViewer = ({ message, mode, onToggleStar, onDelete }) => {
  ...
  // [에러 해결] sender가 객체({userName})인 경우와 문자열인 경우 모두 대응
  const senderName =
    typeof message.sender === "object"
      ? message.sender.userName
      : message.sender;
  const isStarred = message.isStar || message.isStarred;

  // Teams 모드 판단
  const isTeamInvite = message.type === 1 || message.type === "team-invite";
  const isJoinRequest =
    message.type === 2 || message.type === "team-join-request";
  const isDecisionMessage = mode === "teams" && (isTeamInvite || isJoinRequest);

  const inviteDecision =
    inviteDecisionById[message.id || message.invitationId] ?? null;

  const decideInvite = (decision) => {
    setInviteDecisionById((prev) => ({
      ...prev,
      [message.id || message.invitationId]: decision,
    }));
    alert("완료되었습니다.");
  };
    ...
    <section className="flex min-h-[640px] flex-1 flex-col rounded-2xl border border-[#E4E9F2] bg-white p-8 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div className="rounded-xl border border-[#EDE6C8] bg-[#FFFDF2] px-4 py-3 text-base leading-7 text-[#656D7E]">
          <p>
            ✉ 보낸 사람: {senderName} (
            {message.role || (isTeamInvite ? "팀장" : "개발자")})
          </p>
          <p>⏰ 날짜: {message.send_at || message.date}</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <ActionCircle
            color="#F16A6A"
            onClick={() => onDelete(message.id || message.invitationId)}
          >
            ✕
          </ActionCircle>
        </div>
      ...
      <article className="mt-8 space-y-7 text-xl leading-[1.95] text-[#4C5568]">
        {/* 본문 렌더링: body가 배열인 경우와 content(문자열)인 경우 모두 대응 */}
        {message.body ? (
